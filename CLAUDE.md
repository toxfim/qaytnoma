...
second build reinstalled the other app's dependencies from scratch. Both
targets are copied to the landing page, which offers them side by side under
*Ikki nashr*; `download/meta.json` is keyed by app (`apps.qaytnoma`,
`apps["qaytnoma-ai"]`) and each build REWRITES ONLY ITS OWN ENTRY — the two
installers are built by separate commands, so writing the whole file would
drop the other app from the page. `deploy-installer.mjs` uploads every app in
that file (or just the one named as its argument, to avoid pushing 132 MB for
an unchanged build). The AI installer is
